<template>
  <div id="main" class="echart-map"></div>
</template>
  ...
  export default {
    ...
    methods: {
      initMap() {
       ...
       let options = this.mapOption()

        let behavior = echarts.init(document.getElementById('main'));
        /*let options = {
          tooltip: {},
      ...
      mapOption(){
        ...
        // 发射点和线路颜色
        let color = ['rgba(246, 146, 26, .8)', '#a6c84c', '#46bee9'] // 橙 墨绿 蓝
        let seriesData = []
        lineDataArr.forEach(function (item, i) {
          seriesData.push(
            ...
            0, 0, 0, 1,
            [
              {offset: 0, color: 'rgba(174, 117, 41, 1)'},
              {offset: 0.5, color: 'rgba(76, 146, 165, 1)'},
              {offset: 1, color: 'rgba(0, 146, 229, 1)'}
            ]
          ),
                  width: 0,
                  curveness: 0.2
